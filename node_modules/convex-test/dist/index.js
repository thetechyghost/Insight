...
function asyncSyscallImpl() {
    return async (op, jsonArgs) => {
        ...
        switch (op) {
            ...
            case "1.0/runUdf": {
                const { udfType, name, reference, functionHandle, args: udfArgsJson, } = args;
                ...
                if (udfType === "query") {
                    return JSON.stringify(convexToJson(await withAuth().queryFromPath(functionPath, 
                    /* isNested */ true, udfArgs)));
                }
                if (udfType === "mutation") {
...
function withAuth(auth = new AuthFake()) {
    ...
    const byType = {
        query: async (functionReference, args) => {
            ...
            return await byTypeWithPath.queryFromPath(functionPath, 
            /* isNested */ false, args);
        },
        mutation: async (functionReference, args) => {
            const functionPath = await getFunctionPathFromReference(functionReference);
            return await byTypeWithPath.mutationFromPath(functionPath, 
            /* isNested */ false, args);
        },
